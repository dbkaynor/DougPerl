...
my %window  = (
    width  => $pane{width} + $canvas{width} + 4 * $padding,
...
my $pix_per_pt = 2;
my %num_of_pts = (
    hor => $canvas{width} / $pix_per_pt,
    ver => $canvas{height} / $pix_per_pt
);

my %plot_range =
  ( hor => { from => -2.0, to => 2.0 }, ver => { from => -2.0, to => 2.0 } );
my %resolution = (
    hor => ( $plot_range{hor}{to} - $plot_range{hor}{from} ) / $num_of_pts{hor},
    ver => ( $plot_range{ver}{to} - $plot_range{ver}{from} ) / $num_of_pts{ver}
);
...
$pane->Button( -text => "Exit", -command => sub { exit } )->pack(
    -side => "bottom",
    -pady => $padding
);
...
$canvas->Tk::bind(
    "<Motion>",
    [
        \&track_motion, Ev('x'),      Ev('y'),    $pix_per_pt,
        \%resolution,   \%plot_range, \$xy_label, \$val_label
    ]
);
...
for ( my $y = 0 ; $y < $canvas{height} ; $y += $pix_per_pt ) {
    ...
    for ( my $x = 0 ; $x < $canvas{width} ; $x += $pix_per_pt ) {
        ...
        my $color =
          sprintf( "#%x%x%x", 255 * $value, 255 * $value, 255 * $value );
        $canvas->createRectangle(
            $x, $y, $x + $pix_per_pt, $y + $pix_per_pt,
            -fill    => $color,
            -outline => $color
        );
    }
...
sub track_motion {
    my ( undef, $x, $y, $pix_per_pt, $r_res, $r_range, $r_xy_label,
        $r_val_label )
      = @_;
