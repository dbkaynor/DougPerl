...
my $Autocircles = FALSE;
my $Circumference = TRUE;
...
$B1_frame->Button( -text => 'Draw', -command => \&Draw_bttn) ->pack( -side => 'left', -fill => 'both', -expand => '1' );
$B1_frame->Button( -text => 'Clear', -command => \&Clear_bttn) ->pack( -side => 'left', -fill => 'both', -expand => '1'  );
...
$B3_frame->Button(-text => 'Right', -command => sub {$canvas->addtag('everything', 'all');
  $canvas->move('all',  $mwY / 20, 0)}) ->pack( -side => 'left', -fill => 'both', -expand => '1' );
...
my $Repeat_scale = $One_frame->Scale(-orient=>'horizontal', -variable => \$AutoRepeat, -width => 8, -label => 'Repeat delay') ->pack( -side => 'top' );
...
my @CirclesChoices = qw/2 3 4 5 6 7/;
$Four_frame->BrowseEntry(-label => "Circles", -choices => \@CirclesChoices, -variable => \$Circles, -width => 5)->pack( -side => 'top' );
$Four_frame->Checkbutton(-text => 'Auto circles', -variable => \$Autocircles) ->pack( -side => 'top' );

$Four_frame->Checkbutton(-text => 'Draw circumference', -variable => \$Circumference ) ->pack( -side => 'top' );
...
sub GetPoint($)
{
    ...
    my @Out;
    $Out[0] = (cos($AngleR)*$Multiplier)+$XOffset;  #X value
    $Out[1] = (sin($AngleR)*$Multiplier)+$YOffset;  #Y value
    $Out[2] = $AngleR; #Radians
    $Out[3] = $AngleD; #Degrees
    return @Out;
}
#---------------------------------------------------------------------------
sub GetCircleData($)
...
sub DrawCircle($) {
   ...
   for (my $x = 0; $x < $Y; $x+=4) {
       ...
       if ($t4 == 0) {$text = '#'};
       if ($x == 0) {$text = '%'};
        $canvas->createText($t1,$t2,-text=>$text,-fill=>$tcolor,-anchor=>"center");
       }
    }
    if ($Circumference) {
       push @list, $list[0], $list[1];
       $canvas->createLine (@list,-fill => $tcolor,-width => $LineWidth)
    }
    $mw->update();
}
...
sub DrawLines($$) {
   ...
   for (my $x = 0; $x < $Y; $x+=4) {
       ...
       my $u3 = @$array2[$x+2];
       my $u4 = @$array2[$x+3];
       if ($VaryFGColor) {$FGColor = RandColor; $t = $FGColor};
       $canvas->createLine ($t1,$t2,$u1,$u2,-fill => $t,-width=>$LineWidth);
    }
    $mw->update();
}
#---------------------------------------------------------------------------
sub DoTheShift($) {
     my ($array) = shift;
     $Shift = int(rand(360/$Degrees));
     my @t = splice @$array, 0, $Shift * 4;
     push @$array,@t;
}
#---------------------------------------------------------------------------
sub Clear_bttn {
...
#---------------------------------------------------------------------------
#draw between 2 and 5 circles
#The Degrees are always the same, the multiplier, x and y offset may vary\
sub Draw_bttn {
  do {
      Log("----- Draw BUTTON -----");
      @A0 = ();
